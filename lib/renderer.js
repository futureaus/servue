...
class Renderer {
     ...
     * @param {boolean} hydrate - Should hydrate on mount?
     */
    async createEntry(filePath, hydrate) {
    ...
    async getRenderer(filePath, hydrate) {
        ...
        if(this.cache[filePath]){
            ...
            if(!this.resources) throw new Error('servue.resources is not set, make sure the directory is provided')
     ...
     */
    async render(vueFile, data = {}, hydrate = true) {
    ...
    defaultContext(data){
        return {htmlattr: '', head: '', data: data}
    }
